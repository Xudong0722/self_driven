...
从输出可以看出来，obj1调用的是拷贝构造，obj2调用的是移动构造，原因是std::move(obj1) 这里返回了一个将亡值，在这里它是右值，所以这里会调用移动构造，而对于`Test obj1 = rref_test;` rref_test是一个左值，所以调用的是拷贝构造，我们可以借助汇编代码验证：
...
```c
        ...
        lea     rax, [rbp-32]
```
这里可以看出来这里产生了一个临时的右值，所以我们调用的是移动构造函数。