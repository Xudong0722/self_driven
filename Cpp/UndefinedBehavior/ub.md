...
- [未定义行为？](#未定义行为)
  ...
  - [指针别名类](#指针别名类)
    - [reinterpret\_cast 后以不兼容的类型访问](#reinterpret_cast-后以不兼容的类型访问)
    - [union 访问不是激活的成员](#union-访问不是激活的成员)
    - [T 类型指针必须对齐到 alignof(T)](#t-类型指针必须对齐到-alignoft)
  - [算数类](#算数类)
  ...
  - [生命周期类](#生命周期类)
    ...
    - [new / new\[\] / malloc 和 delete / delete\[\] / free 必须匹配](#new--new--malloc-和-delete--delete--free-必须匹配)
  ...
  - [库函数类](#库函数类)
    ...
    - [v.back() 当 v 为空时是未定义行为](#vback-当-v-为空时是未定义行为)
    - [vector 的 operator\[\] 当 i 越界时，是未定义行为](#vector-的-operator-当-i-越界时是未定义行为)
    - [容器迭代器失效](#容器迭代器失效)
    - [容器元素引用失效](#容器元素引用失效)
  - [多线程类](#多线程类)
    ...
    - [对于非 recursive\_mutex，同一个线程对同一个 mutex 重复上锁，会产生未定义行为（俗称递归死锁）](#对于非-recursive_mutex同一个线程对同一个-mutex-重复上锁会产生未定义行为俗称递归死锁)
- [总结](#总结)

https://github.com/parallel101/course/blob/master/slides/undef/README.md
